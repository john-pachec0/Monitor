#!/bin/bash
# Untwist Package Verification Script
# Run this after extracting to verify all files are present

echo "🔍 Verifying Untwist Package..."
echo ""

# Check if we're in the right directory
if [ ! -f "UntwistApp.swift" ]; then
    echo "❌ Error: UntwistApp.swift not found"
    echo "   Please run this script from the Untwist/ directory"
    exit 1
fi
