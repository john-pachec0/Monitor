# App Store Metadata for Untwist

**Last Updated:** November 10, 2024

This document contains all the text content needed for the App Store listing.

---

## App Name

**Primary:** `Untwist - CBT Thought Journal`

**Alternatives** (if primary is taken):
- `Untwist: CBT for Anxiety`
- `Untwist - Anxiety Relief`
- `Untwist: Thought Reframing`

**Character Limit:** 30 characters (Primary option is 27 characters ✅)

---

## Subtitle

`Privacy-first anxiety relief`

**Alternative subtitles:**
- `Reframe anxious thoughts privately`
- `CBT journal for anxious thoughts`
- `Private thought reframing tool`

**Character Limit:** 30 characters (Primary option is 27 characters ✅)

---

## Promotional Text

*(This appears at the top of your listing and can be updated without submitting a new version)*

```
Your mental health data belongs to you, and only you. Untwist is the only CBT app that keeps everything on your device—no accounts, no cloud, no tracking. Practice evidence-based thought reframing techniques in complete privacy.
```

**Character Limit:** 170 characters (This is 216 characters - needs trimming)

**Shorter version (169 characters):**
```
Practice CBT techniques in complete privacy. No accounts, no cloud, no tracking. Your mental health data stays on your device, where it belongs.
```

---

## Description

*(4000 character maximum - this is ~3,800 characters)*

```
PRIVACY-FIRST ANXIETY RELIEF

Your anxious thoughts belong to you, not a company's servers. Untwist is built differently—everything stays on your device. No accounts, no cloud sync, no tracking. Just you and evidence-based cognitive behavioral therapy (CBT) techniques.

WHAT IS UNTWIST?

Untwist helps you capture and reframe anxious thoughts using proven CBT methods. When anxiety strikes, quickly capture the thought. Later, during your scheduled "worry time," review and reframe it using techniques from cognitive behavioral therapy.

KEY FEATURES

• Quick Capture
  When anxiety hits, capture the thought in seconds. Don't let it spiral—just write it down and move on with your day.

• Scheduled Review Time
  Set your "worry time" for when it works best (evenings work well). Postponing review often reduces anxiety naturally—many worries lose intensity over time.

• Guided Reframing
  Step-by-step process helps you:
  - Identify cognitive distortions in your thinking
  - Challenge unhelpful thought patterns
  - Create balanced, realistic reframes
  - Track how your anxiety changes

• Learn CBT Techniques
  Understand the 13 cognitive distortions with clear explanations, real-world examples, and practical reframing tips. Learn to spot patterns in your thinking.

• Worry Box
  Your private collection of captured thoughts, ready for review when the time is right. Think of it as a holding space for anxiety.

• Pattern Recognition
  Over time, notice patterns in your worries. See which cognitive distortions you tend toward, and build awareness of your thinking habits.

PRIVACY YOU CAN TRUST

• ✓ All data stays on your device
• ✓ No account required
• ✓ No cloud sync or backups
• ✓ No tracking or analytics
• ✓ No advertising
• ✓ Encrypted at rest with iOS device encryption
• ✓ We can't access your data—even if we wanted to

Your thoughts, reframes, and mental health data never leave your iPhone. We built Untwist this way intentionally because privacy matters, especially for mental health.

WHY POSTPONE WORRY?

Research shows that scheduling worry time (rather than worrying constantly) can reduce overall anxiety. Benefits include:

• Many worries resolve themselves or become less intense
• You can give worries focused attention rather than scattered rumination
• You're not interrupted by anxiety throughout the day
• You build a healthier relationship with anxious thoughts

BASED ON COGNITIVE BEHAVIORAL THERAPY

CBT is one of the most researched and effective approaches for anxiety. Untwist helps you practice core CBT skills:

• Thought awareness - Notice when anxiety shows up
• Cognitive restructuring - Challenge and reframe thoughts
• Pattern recognition - Identify thinking traps
• Evidence evaluation - Separate thoughts from facts

THIS IS NOT THERAPY

Untwist is a self-help tool, not a replacement for professional mental health care. If you're experiencing severe anxiety, depression, or thoughts of self-harm, please reach out to a mental health professional.

PERFECT FOR

• Anyone experiencing anxious thoughts
• People learning CBT techniques
• Therapy clients practicing between sessions
• Privacy-conscious users who want control over their mental health data
• People who want to try CBT before committing to therapy

WHY WE BUILT THIS

Mental health data is incredibly sensitive. Yet most apps sync it to the cloud, create accounts, and track everything you do. We built Untwist because we believe your thoughts belong to you—not to servers, not to companies, not to anyone else.

EVIDENCE-BASED & TRANSPARENT

Untwist is based on established CBT research and principles. We're transparent about what the app does (and doesn't do), and we'll never make exaggerated claims about results.

Download Untwist and start reframing anxious thoughts in complete privacy.

---

Questions? Feedback? Email us at [your email]

Privacy Policy: [your privacy policy URL]
```

---

## Keywords

*(100 character maximum, comma-separated)*

**Primary Keywords (78 characters):**
```
CBT, anxiety, cognitive distortions, thought journal, worry journal, privacy
```

**Extended Keywords (95 characters):**
```
CBT, anxiety, cognitive distortions, thought journal, worry journal, mental health, therapy, privacy
```

**Keyword Research Notes:**
- **High priority:** CBT, anxiety, thought journal, cognitive distortions
- **Medium priority:** mental health, therapy, reframe, worry
- **Long-tail:** privacy-first mental health, CBT for anxiety
- **Avoid:** therapy app (we're not therapy), diagnose (we don't diagnose)

---

## Categories

**Primary Category:** Health & Fitness
**Secondary Category:** Medical

**Rationale:**
- Health & Fitness is broader, better for discovery
- Medical signals clinical/evidence-based approach
- Both are appropriate for mental health tools

---

## Age Rating

**Recommended:** 12+

**Reasons for 12+:**
- Mental health content (anxiety, distortions)
- Text about potentially distressing thoughts
- Crisis resources mention suicide/self-harm

**Not 4+ because:**
- Content is designed for older teens and adults
- Requires reading comprehension and self-reflection
- Deals with mature mental health topics

**Alternative:** 17+ if Apple requires it for mental health content

---

## What's New (For Future Updates)

*(Version 1.0 - Initial Release)*

```
Welcome to Untwist!

Untwist helps you capture and reframe anxious thoughts using evidence-based CBT techniques—all while keeping your data completely private on your device.

Features:
• Quick thought capture
• Scheduled worry time
• Guided CBT reframing process
• Learn about 13 cognitive distortions
• Pattern recognition over time
• Complete privacy—no accounts, no cloud

Your mental health data stays with you. No tracking, no analytics, no servers.

Questions or feedback? Tap Settings → Send Feedback
```

**Character Limit:** 4000 characters (This is ~500 characters ✅)

---

## Support URL

`https://yourdomain.com/untwist/support`

**Or use GitHub repo:**
`https://github.com/yourusername/untwist`

**Must be live before submission.**

---

## Marketing URL (Optional)

`https://yourdomain.com/untwist`

**Could include:**
- Overview of the app
- Privacy commitment
- Screenshots
- Download link
- FAQ

---

## Privacy Policy URL (Required)

`https://yourdomain.com/untwist/privacy`

**Must be:**
- Publicly accessible (no login required)
- Up to date
- Matches the content in PRIVACY_POLICY.md
- Live before submission

---

## Copyright

`© 2024 John Pacheco. All rights reserved.`

---

## App Review Notes (For Apple)

*(These notes help Apple's review team understand your app)*

```
TESTING NOTES FOR APPLE REVIEW

Thank you for reviewing Untwist. Here's what you need to know:

CORE FUNCTIONALITY:
Untwist is a thought journaling and reframing tool based on CBT (Cognitive Behavioral Therapy) techniques. Users capture anxious thoughts and review them later using guided reframing.

HOW TO TEST:
1. Complete onboarding (3 quick screens)
2. Tap "Capture a Thought" and enter any anxious thought
3. The thought is saved to the Worry Box
4. Navigate to Worry Box → tap a thought to review it
5. Follow the guided review process:
   - Rate anxiety level
   - Select cognitive distortions
   - Write a reframe
   - Rate anxiety level again
6. Explore Learn section to see educational content

PRIVACY IMPLEMENTATION:
- All data stored locally using SwiftData (iOS 17+)
- No network calls except optional feedback (Settings → Send Feedback)
- No tracking, analytics, or third-party SDKs
- No account creation or user authentication

FEEDBACK API:
- Optional feedback can be sent from Settings → Send Feedback
- This is the ONLY network call in the app
- API endpoint: https://api.untwist.app/feedback
- Requires API key authentication
- Rate limited to prevent abuse

MENTAL HEALTH DISCLAIMERS:
- Disclaimers are shown during onboarding (page 4)
- Crisis resources available in Settings
- Clear "not a substitute for therapy" messaging
- 988 Suicide & Crisis Lifeline prominently featured

NOTIFICATIONS:
- Optional local notifications can be scheduled
- User sets their preferred "worry time" in Settings
- Notifications use iOS UserNotifications framework (local only)
- No remote notifications or push servers

NO ACCOUNT/LOGIN:
- App works completely offline
- No user authentication of any kind
- No personal information collected

TEST ACCOUNT: Not applicable (no accounts)

ISSUES WE'RE AWARE OF:
- None at this time

If you have questions during review, please contact: [your email]

Thank you!
```

---

## Screenshots Requirements

**Required Sizes:**
- **6.7"** (iPhone 15 Pro Max): 1290 x 2796 pixels (required)
- **6.5"** (iPhone 11 Pro Max): 1284 x 2778 pixels (optional but recommended)
- **5.5"** (iPhone 8 Plus): 1242 x 2208 pixels (required if supporting older devices)

**Number:** 3-10 screenshots (recommend 6-8)

**Recommended Screenshots:**
1. **Home Screen** - Clean, centered message with capture button
2. **Capture Flow** - Quick thought capture interface
3. **Worry Box** - Thoughts ready for review
4. **Review Process** - Guided distortion selection
5. **Reframe Editor** - Writing a balanced reframe
6. **Learn Section** - Educational distortion cards
7. **Privacy Focus** - Highlight "all data stays on your device"
8. **Settings** - Customization options

**Design Notes:**
- Use device frames (iPhone mockups)
- Include short captions on each screenshot
- Show both light and dark mode (optional but nice)
- Keep UI clean, readable, professional
- Emphasize privacy messaging

---

## App Preview Video (Optional)

**Recommended:** Yes, videos increase conversion by ~20-30%

**Length:** 15-30 seconds (max 30 seconds)

**Content:**
1. Show anxiety → capture thought (3-4 seconds)
2. Show worry box with thoughts (3-4 seconds)
3. Show review process (5-6 seconds)
4. Show reframe creation (5-6 seconds)
5. End with privacy message: "All data stays on your device" (4-5 seconds)

**Requirements:**
- Same aspect ratios as screenshots
- No sound required (but recommended)
- Accessible captions if there's voice
- Professional quality (no phone recordings)

---

## SEO & Discovery Strategy

**App Store Optimization (ASO) Tips:**

1. **Title matters most**
   - Include "CBT" (searchable term)
   - "Thought Journal" is less competitive than "anxiety app"
   - "Untwist" is unique, memorable

2. **Subtitle is secondary ranking**
   - "Privacy-first" differentiates
   - "anxiety relief" is searchable

3. **Keywords drive search**
   - Don't repeat words from title/subtitle in keywords
   - Use all 100 characters
   - Focus on what users search for

4. **Description first 170 characters are critical**
   - Lead with unique value (privacy)
   - Front-load important keywords
   - Make it compelling

5. **Ratings & reviews boost ranking**
   - Encourage happy users to review
   - Respond to all reviews
   - Fix issues users mention

6. **Update regularly**
   - Frequent updates signal active development
   - Each update resets "What's New" to top
   - Can update promotional text anytime

---

## Localization (Future)

**v1.0:** English only

**Future considerations:**
- Spanish (large US market)
- French, German (Europe)
- Japanese (Asia)
- Portuguese (Brazil)

**What to localize:**
- App Store metadata (description, keywords, screenshots)
- In-app text (all UI strings)
- Educational content (distortion guides)
- Crisis resources (country-specific)

---

## Pricing

**Free** (no in-app purchases in v1.0)

**Future monetization options (if needed):**
- One-time purchase for Pro features ($4.99-$9.99)
- Donation/tip jar
- Export/backup features

**Why free:**
- Mental health tools should be accessible
- Build user base first
- Prove value before charging
- Privacy-first model reduces operating costs

---

## Notes for App Store Connect

**Build Number:** 1.0.0 (update based on actual build)
**Bundle ID:** com.johnpacheco.Untwist
**SKU:** untwist-ios-app

**Content Rights:** You own the rights to all content

**Encryption:** No (SwiftData is local only, HTTPS is standard)

**Advertising Identifier:** No (we don't use IDFA)

**Third-Party Content:** No (all original content)

---

## Launch Checklist

Before submitting to App Store:
- [ ] Privacy Policy is live at URL
- [ ] Terms of Service is live at URL
- [ ] Support URL is live and functional
- [ ] All screenshots are created and uploaded
- [ ] App icon is final and looks good
- [ ] Description is proofread (no typos!)
- [ ] Keywords are optimized
- [ ] App Review Notes are complete
- [ ] Test build works on physical device
- [ ] All disclaimers are clear
- [ ] Crisis resources are accurate

---

**Next Steps:**
1. Host Privacy Policy and Terms at public URLs
2. Create App Store screenshots (use iPhone simulator)
3. Optional: Create app preview video
4. Fill in App Store Connect with this metadata
5. Submit for review!

---

**Questions?** Reference this document when filling out App Store Connect.

**Last Updated:** November 10, 2024
